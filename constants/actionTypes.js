//RootApp.js
//reducer/user.js
export const SET_CURRENT_USER = "SET_CURRENT_USER";

//container/Login/SignIn.js
//reducer/user_search.js
export const USER_SEARCH = "USER_SEARCH";
export const USER_SEARCH_SUCCESS = "USER_SEARCH_SUCCESS";
export const USER_SEARCH_FAIL = "USER_SEARCH_FAIL";

//container/Login/SignIn.js
//reducer/user.js
export const AUTH_LOGIN = "AUTH_LOGIN";
export const AUTH_LOGIN_SUCCESS = "AUTH_LOGIN_SUCCESS";
export const AUTH_LOGIN_FAIL = "AUTH_LOGIN_FAIL";

//container/Settings/SettingsOverview.js
//reducer/user.js
export const AUTH_LOGOUT = "AUTH_LOGOUT";
export const AUTH_LOGOUT_ALL = "AUTH_LOGOUT_ALL";
export const AUTH_LOGOUT_ALL_SUCCESS = "AUTH_LOGOUT_ALL_SUCCESS";
export const AUTH_LOGOUT_ALL_FAIL = "AUTH_LOGOUT_ALL_FAIL";

//container/Login/SignUp.js
//reducer/user.js
export const AUTH_REGISTER = "AUTH_REGISTER";
export const AUTH_REGISTER_SUCCESS = "AUTH_REGISTER_SUCCESS";
export const AUTH_REGISTER_FAIL = "AUTH_REGISTER_FAIL";

//container/Create/*
//reducer/create_poll.js & reducer/polls.js
export const CREATE_POLL_CHANGE_POLLTYPE = "CREATE_POLL_CHANGE_POLLTYPE";
export const CREATE_POLL = "CREATE_POLL";
export const CREATE_POLL_SUCCESS = "CREATE_POLL_SUCCESS";
export const CREATE_POLL_FAIL = "CREATE_POLL_FAIL";
export const CREATE_POLL_INITIALSTATE = "CREATE_POLL_INITIALSTATE";

//container/Polls/*
//reducer/reducer/polls.js
export const LOAD_POLLS_HOME = "LOAD_POLLS_HOME";
export const LOAD_POLLS_HOME_SUCCESS = "LOAD_POLLS_HOME_SUCCESS";
export const LOAD_POLLS_HOME_FAIL = "LOAD_POLLS_HOME_FAIL";

//container/Polls/*
//reducer/polls.js
export const VOTE_FROM_HOME = "VOTE_FROM_HOME";
export const VOTE_FROM_HOME_SUCCESS = "VOTE_FROM_HOME_SUCCESS";
export const VOTE_FROM_HOME_FAIL = "VOTE_FROM_HOME_FAIL";
export const VOTE_FROM_USER = "VOTE_FROM_USER";


//reducer/other_users.js
export const SAVE_USER_PROFILE = "SAVE_USER_PROFILE";
export const CLEAR_USER_PROFILES = "CLEAR_USER_PROFILES";
export const LOAD_USER_PROFILE = "LOAD_USER_PROFILE";
export const SAVE_USER_PROFILE_FAIL = "SAVE_USER_PROFILE_FAIL";

//follow
export const FOLLOW_USER = "FOLLOW_USER";
export const FOLLOW_USER_SUCCESS = "FOLLOW_USER_SUCCESS";
export const FOLLOW_USER_FAIL = "FOLLOW_USER_FAIL";

export const UPLOAD_PB = "UPLOAD_PB";
export const UPLOAD_PB_SUCCESS = "UPLOAD_PB_SUCCESS";
export const UPLOAD_PB_FAIL = "UPLOAD_PB_FAIL";


export const UPDATE_USER_FROM_API = "UPDATE_USER_FROM_API";
export const UPDATE_USER_FROM_API_SUCCESS = "UPDATE_USER_FROM_API_SUCCESS";
export const UPDATE_USER_FROM_API_FAIL = "UPDATE_USER_FROM_API_FAIL";

